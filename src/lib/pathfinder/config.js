...
const readDecode = fname => (Buffer.from(fs.readFileSync(fname, 'ascii'), 'ascii'))
...
class Config {
  constructor () {
    ...
    this.env = env
    // Our secret key
    this.keyPath = process.env.CLIENT_KEY_FILENAME || `${root}/secrets_${env}/clientkey.pem`
    // The cert we'll use to identify ourselves to the pathfinder service
    this.certPath = process.env.CLIENT_CERT_FILENAME || `${root}/secrets_${env}/clientcert.pem`
    // Pathfinder's cert, we'll use this to authenticate pathfinder

    this.caPath = process.env.CA_CERT_FILENAME || `${root}/secrets_${env}/pathfindercert.pem`
    if (env === 'live') {
      this.caPath = process.env.CA_CERT_FILENAME || `${root}/secrets_live/pathfindercert_root_cert.pem`
      this.caPathIntermediate = process.env.CA_CERT_INTERMEDIATE_FILENAME || `${root}/secrets_live/pathfindercert_intermediate_cert.pem`
    }
