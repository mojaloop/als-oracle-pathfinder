...
module.exports = class TlsResolver {
  ...
  // TODO: comments
  _genMessageId () {
    let id; const min = DNS_MIN_ID; const max = DNS_MAX_ID; const maxAttempts = max - min; let numAttempts = 0
  ...
  _handleSocketData (data) {
    ...
    if (this.data.response.byteLength >= this.data.expectedLength) {
      const resp = dnsPacket.streamDecode(this.data.response)
  ...
  _sendMessage (message) {
    ...
    // encode the packet
    const buf = dnsPacket.streamEncode(message.request)
